...
f_output=open("nboutput.txt","w", encoding="utf8")

modelDict=json.load(open('nbmodel.txt'))
classPriorProbDict=copy.deepcopy(modelDict['Class Prob'])
wordClassProbDict=copy.deepcopy(modelDict['Word Probabilites'])
wordCountDict=copy.deepcopy(modelDict['Word Counts'])
highFreq=list()
lowFreq=list()
highFreqWords=list()
lowFreqWords=list()
...
def findHighFreqWords():
	...
	highFreq = sorted(wordCountDict.items(), key=operator.itemgetter(1), reverse=True)[:16]
	for pair in highFreq:
		highFreqWords.append(pair[0])


def findClassTag(true, fake, pos, neg):
	if true>fake:
		classTag1="True"
	else:
		classTag1="Fake"
...
def processSentence():
	...
	for sentence in sentences:
		...
		for word in sentence:
			...
			if ((word not in wordClassProbDict) or (word in highFreqWords)): 
				true=true+0

			else:
				true=true+math.log(wordClassProbDict[word]['True'])
				pos=pos+math.log(wordClassProbDict[word]['Pos'])
				neg=neg+math.log(wordClassProbDict[word]['Neg'])
				fake=fake+math.log(wordClassProbDict[word]['Fake'])


		true=true+math.log(classPriorProbDict['True'])
		neg=neg+math.log(classPriorProbDict['Neg'])
		pos=pos+math.log(classPriorProbDict['Pos'])
		fake=fake+math.log(classPriorProbDict['Fake'])

		classTag1, classTag2=copy.deepcopy(findClassTag(true, fake, pos, neg))
